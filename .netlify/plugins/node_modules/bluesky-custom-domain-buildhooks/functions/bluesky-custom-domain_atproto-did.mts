...
var resolver = (wretch) => {
  ...
  const { _url: url, _options: opts, _config: config3, _catchers, _resolvers: resolvers, _middlewares: middlewares, _addons: addons } = wretch;
  const catchers = new Map(_catchers);
  const finalOptions = mix(config3.options, opts);
  let finalUrl = url;
  const _fetchReq = middlewareHelper(middlewares)((url2, options) => {
    finalUrl = url2;
    return config3.polyfill("fetch")(url2, options);
  })(url, finalOptions);
  ...
  const throwingPromise = _fetchReq.catch((error) => {
    ...
    if (!response.ok) {
      ...
      const jsonErrorType = config3.errorType === "json" || ((_a = response.headers.get("Content-Type")) === null || _a === void 0 ? void 0 : _a.split(";")[0]) === "application/json";
      const bodyPromise = !config3.errorType ? Promise.resolve(response.body) : jsonErrorType ? response.text() : response[config3.errorType]();
...
var NetlifyClient = class {
  ...
  constructor(apiKey, { baseUrl = "" } = {}) {
    this.apiKey = apiKey;
    this.wretch = dist_default().url(baseUrl !== "" ? baseUrl : NETLIFY_API_URL).auth(`Bearer ${apiKey}`).headers({ "Api-Version": "2" });
  }
  /**
...
var NetlifyExtensionClient = class extends NetlifyClient {
   ...
   */
  upsertTeamConfiguration = async (teamId, config3) => {
    return this.extensionApi.url(`/team/${teamId}/integrations/${this.extension}/configuration/team`).put({ config: config3 }).res();
  };
  /**
   * Creates a configuration object of type 'team' for the extension installation
   */
  createTeamConfiguration = async (teamId, config3) => {
    return this.extensionApi.url(`/team/${teamId}/integrations/${this.extension}/configuration/team`).put({ config: config3 }).res();
  };
  /**
   * Updates a configuration object of type 'team' for the extension installation
   */
  updateTeamConfiguration = async (teamId, config3) => {
    return this.extensionApi.url(`/team/${teamId}/integrations/${this.extension}/configuration/team`).put({ config: config3 }).res();
  };
  /**
   ...
   */
  upsertSiteConfiguration = async (teamId, siteId, config3) => {
    return this.extensionApi.url(`/team/${teamId}/integrations/${this.extension}/configuration/site/${siteId}`).put({ config: config3 }).res();
  };
  /**
   * Creates a configuration object of type 'site' for the extension installation
   */
  createSiteConfiguration = async (teamId, siteId, config3) => {
    return this.extensionApi.url(`/team/${teamId}/integrations/${this.extension}/configuration/site/${siteId}`).put({ config: config3 }).res();
  };
  /**
   * Updates a configuration object of type 'site' for the extension installation
   */
  updateSiteConfiguration = async (teamId, siteId, config3) => {
    return this.extensionApi.url(`/team/${teamId}/integrations/${this.extension}/configuration/site/${siteId}`).put({ config: config3 }).res();
  };
  /**
   ...
   */
  createConnectConfiguration = async ({ teamId, dataLayerId, config: config3, name, prefix }) => {
    return this.extensionApi.url(`/team/${teamId}/integrations/${this.extension}/configuration/connect/${dataLayerId}`).post({ name, prefix, config: config3 }).res();
  };
  /**
   ...
   */
  updateConnectConfiguration = async ({ teamId, dataLayerId, configurationId, name, prefix, config: config3 }) => {
    return this.extensionApi.url(`/team/${teamId}/integrations/${this.extension}/configuration/connect/${dataLayerId}/${configurationId}`).patch({ name, prefix, config: config3 }).res();
  };
  /**
   ...
   */
  createVisualEditorConfiguration = async ({ teamId, siteId, config: config3, name, projectId, prefix }) => {
    return this.extensionApi.url(`/team/${teamId}/integrations/${this.extension}/configuration/create/${siteId}?project_id=${projectId}`).post({ name, config: config3, prefix }).res();
  };
  /**
   ...
   */
  updateVisualEditorConfiguration = async ({ teamId, siteId, configurationId, config: config3, name, projectId, prefix }) => {
    return this.extensionApi.url(`/team/${teamId}/integrations/${this.extension}/configuration/create/${siteId}/${configurationId}?project_id=${projectId}`).patch({ name, config: config3, prefix }).res();
  };
  /**
...
// node_modules/@netlify/sdk--ui-functions/dist/sdk_context.js
var SDKContextProviderTokensSchema = z.array(z.object({
  token: z.string().min(1).trim(),
  externalId: z.string().min(1).trim(),
  connectionId: z.string().min(1).trim()
}));
var parseProviderTokens = (header) => {
  if (!header) {
    return [];
  }
  try {
...
var createNetlifySDKContext = ({ req }) => {
  const netlifyToken = req.headers.get("Nf-UIExt-Netlify-Token");
  const slug = req.headers.get("Nf-UIExt-Extension-Slug");
  const client = new NetlifyExtensionClient(
    netlifyToken ?? "",
    slug ?? "",
    // FIXME(ndhoule): This ain't great, we really should bail out slug is not available
    { netlifyApiUrl: process.env["NETLIFY_API_BASE_URL"] }
  );
...
// node_modules/@netlify/sdk--ui-functions/dist/with_netlify_sdk_context.js
var withNetlifySDKContext = (handler) => {
  return (req, context) => handler(req, {
    ...context,
    ...createNetlifySDKContext({
      req,
      context
    })
  });
...
// src/functions/atproto-did.mts
var atproto_did_default = withNetlifySDKContext(async (req, context) => {
...
export {
  config2 as config,
  atproto_did_default as default
};

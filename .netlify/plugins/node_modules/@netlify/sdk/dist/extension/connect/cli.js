 ...
 */
function getEngineConfigWithBundledPlugin({ cwd, extension, }) {
    const connectorConfig = extension?.netlifyConnectPlugin?.config || {};
    ...
    const localDevOptions = extension?.netlifyConnectPlugin?.config?.localDevOptions;
    const engineConfig = {
...
export async function createDevelopmentContentEngine({ extension, cwd, engineOptions, forceRunVisualEditor, }) {
    const engineConfig = getEngineConfigWithBundledPlugin({
        extension,
        cwd: cwd,
    });
    const hooksPath = join(cwd, `/.ntli/connector/content-engine-hooks.cjs`);
    const useHooks = (extension.netlifyConnectPlugin?.supports?.visualEditor ||
        forceRunVisualEditor) &&
        existsSync(hooksPath);
    engineOptions = {
        engineConfig: { ...engineConfig, ...(engineOptions?.engineConfig || {}) }, // this configures plugin settings for the engine. developers that use the SDK specify these when calling extension.addConnector({ localDevOptions: { ... } })
    ...
    // in a development environment that's not the `netlify-extension` CLI (e.g: Stackbit dev)
    engineOptions.env = {
        ...(engineOptions.env || {}),
        SDK_ENV: `development`,
    };
...
export const generateConnectPlugin = async (extensionWorker, config, opts) => {
    const { outDir, watchMode } = opts;
    const connectorDir = resolve(outDir, `connector`);
    if (!(await extensionWorker.hasConnector())) {
    ...
    const normalizedPackageJSON = removeBundledDepsFromPackageJsonContents(packageJSONContents, opts.importedNodeModules);
    ...
    const templateData = {
        // In watch mode stackbit bundles our template file and puts it in a nested subdir (.stackbit/something) because we're simulating a stackbit site inside .ntli to inject our CSI module automatically without the extension dev creating a stackbit.config.js file. So we need ../../
        // In production this isn't the case and the template file isn't rebundled by stackbit, so the import is relative to the extension bundle: ./
        extensionImportPath: `${watchMode ? "../../" : "./"}${rand.toString()}-index.js`,
        configSlug: name,
    ...
    const connectorConfig = await extensionWorker.getConnectorConfig();
    const supportsCreate = connectorConfig.supports?.visualEditor;
...
`);
    ...
    if (!watchMode) {
        ...
        csiFileContents = `import "${templateData.extensionImportPath}"\n${csiFileContents}`;
    }
...
export function getConnectorLocalFrontendConfig(extensionConfig) {
    const localFrontendConfig = extensionConfig?.connector?.["local-frontend"];
    return localFrontendConfig || false;
}
export async function writeStackbitDevFiles({ connectorDir, extensionConfig, connectorConfig, }) {
    const localFrontendConfig = getConnectorLocalFrontendConfig(extensionConfig) || { port: null, directory: null };
    ...
    await writeFile(resolve(connectorDir, "content-engine-hooks.cjs"), javascript `
        ...
        import(join(__dirname, 'index.js')).then(() => {
          const { start } = require(
            require.resolve('@stackbit/dev/dist/dev.js', {
               // check cwd node modules and then the node modules of wherever the SDK is incase we're running the SDK from the SDK repo locally
               paths: [process.cwd(), require.resolve('@netlify/sdk')]
            })
          )
          start({
...
`);
    ...
    if (localFrontendConfig?.directory) {
        ...
        if (configNames.length > 1)
            throw new Error(`Found multiple Netlify Visual Editor configuration files (filenames starting with create.config.*). Only one is allowed.`);
